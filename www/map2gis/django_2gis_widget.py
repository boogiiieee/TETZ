...
class LocationWidget(forms.TextInput):
	...
	def render(self, name, value, *args, **kwargs):
		...
		html += u'<div id="map_%s" style="width:%dpx; height:%dpx"></div>' % (name, self.map_width, self.map_height)
		html += u'<label>%s: </label><input id="address_%s" type="text" style="width:200px" /><br />' % (u'Поиск по адресу', name)
		html += u'<span class="def_address">Текущий адрес: %s</span>' % (address if address else u'(Не указан)',)
